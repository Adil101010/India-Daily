...
(async function () {
  ...
  // 1) Load list (prefer global shared list from main.js)
  let items =
    (window.allArticles && Array.isArray(window.allArticles) && window.allArticles.slice()) ||
    null;

  if (!items) {
    try {
      // ✅ same file name as project: data/article.json
      ...
      showMessage('Content load failed. Check data/article.json and reload the page.');
      return;
    }
  }

  if (!items || !items.length) {
    showMessage('No articles found in data/article.json.');
    return;
  }

  // 2) Pick article (id or slug) or fallback to first
  const idParam = getQueryParam('id') || getQueryParam('slug') || null;
  let article = null;

  if (idParam) {
    article = items.find(
      (it) => String(it.id) === String(idParam) || String(it.slug) === String(idParam)
    );
    if (!article) {
  ...
  // 3) Featured image + OG/Twitter image
  ...
  if (featWrap) {
    ...
    if (article.featuredImage) {
      ...
      (function setImageMeta() {
        ...
        let t = document.querySelector('meta[name="twitter:card"]');
        if (!t) {
          t = document.createElement('meta');
          t.setAttribute('name', 'twitter:card');
          document.head.appendChild(t);
        }
        t.setAttribute('content', 'summary_large_image');
      })();
    }
  }

  // 4) Title, category, author, date, meta desc, canonical
  ...
  if (dateEl) {
    dateEl.textContent = fmtDate(article.publishedAt);
    try {
      dateEl.setAttribute('datetime', new Date(article.publishedAt).toISOString());
    } catch {}
  }
  ...
  if (authorBox) {
    ...
    authorBox.innerHTML = `
      <img
        src="${esc(author.avatar || 'images/default-avatar.png')}"
        alt="${esc(author.name || 'Author')}"
        loading="lazy"
      >
      <div class="info">
    ...
    `;
    // CSS .author-box already display:flex; align-items:center;
  }
  ...
  (function renderRelated() {
    ...
    grid.innerHTML = list
      ...
      <a class="related-card" href="article.html?id=${it.id}">
        ...
        <div>
          <div class="r-body-title">${esc(it.title)}</div>
          <div class="r-body-meta">${fmtDate(it.publishedAt)}</div>
        </div>
      </a>
    `
      )
  ...
  (function addLD() {
    try {
      const ld = {
        ...
        author: {
          '@type': 'Person',
          name: article.author?.name || 'India Daily'
        },
        publisher: {
          '@type': 'Organization',
          name: 'India Daily',
          logo: {
            '@type': 'ImageObject',
            url: 'images/india-daily-logo.jpg'
          }
        },
  ...
  // 10) OG / Twitter minimal tags (title/desc)
  (function setOG() {
    try {
      const setMeta = (type, name, content) => {
        if (!content) return;
        let m = document.querySelector(
          type === 'prop' ? `meta[property="${name}"]` : `meta[name="${name}"]`
        );
        if (!m) {
          m = document.createElement('meta');
          if (type === 'prop') m.setAttribute('property', name);
  ...
  // 12) Reading progress bar (optional: if #readingProgressBar present)
